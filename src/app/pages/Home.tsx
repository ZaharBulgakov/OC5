    ...
    <div>
      ...
      <section className="border-b border-border overflow-hidden relative" style={{ background: "#FFFDF0" }}>
        ...
        <div className="grid lg:grid-cols-[1fr_1fr] min-h-[520px] relative z-10">
          ...
          >
            {/* Gold accent badge */}
            <div className="inline-flex items-center gap-2 mb-5 px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest" style={{ background: "#F5C200", color: "#1A2B4A" }}>
              <span className="w-1.5 h-1.5 rounded-full" style={{ background: "#1A2B4A" }} />
              Официальный сайт
            </div>

            <h1 className="text-4xl lg:text-6xl xl:text-7xl font-bold leading-[1.05] mb-6" style={{ color: "#1A2B4A" }}>
            ...
            <div className="flex flex-wrap justify-center gap-3">
              <Link
                to="/about"
                className="inline-flex items-center gap-2 px-8 py-4 rounded-full transition-all hover:shadow-lg hover:-translate-y-0.5 text-sm font-bold uppercase tracking-wider"
              ...
              <Link
                to="/parents"
                className="inline-flex items-center gap-2 px-8 py-4 rounded-full transition-all text-sm font-bold uppercase tracking-wider hover:-translate-y-0.5"
      ...
      <section className="py-16 px-6 lg:px-10 border-t border-border" style={{ background: "#ffffff" }}>
        <div className="max-w-5xl mx-auto grid lg:grid-cols-2 gap-10 items-center">
          <div className="aspect-[4/3] rounded overflow-hidden border border-border">
            <ImageWithFallback
              src="Director.jpg"
              alt="Директор ОЦ №5"
          ...
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <h2 className="text-3xl lg:text-4xl font-bold mt-3 mb-6" style={{ color: "#1A2B4A" }}>Слово директора</h2>
            <blockquote className="pl-5 space-y-3" style={{ borderLeft: "3px solid #F5C200", color: "#1A2B4A", opacity: 0.75 }}>
              <p>«Мы рады приветствовать вас на официальном сайте нашего центра!»</p>
              <p>«Надеемся на то, что знакомство с нашим образовательным центром будет для вас полезным и интересным!»</p>
              <p>«МАОУ "Образовательный центр № 5 г. Челябинска" – это новые достижения, возможности, перспективы для каждого ученика. Мы развиваем способности всех наших детей и считаем важным, чтобы каждый наш ученик почувствовал себя успешным.»</p>
            </blockquote>
            <p className="mt-5 font-semibold" style={{ color: "#1A2B4A" }}>— Евгений Марченко Геннадьевич</p>
          </motion.div>
        </div>
      </section>

      {/* ─── НОВОСТИ ─── */}
      <section className="py-16 px-6 lg:px-10 border-t border-border" style={{ background: "#1A2B4A" }}>
        <div className="max-w-5xl mx-auto">
          <div className="flex items-end justify-between mb-10">
            <div>
              <p className="text-xs font-mono uppercase tracking-[0.18em] mb-2" style={{ color: "#F5C200" }}>
                Актуальное
              </p>
              <h2 className="text-3xl lg:text-4xl font-bold" style={{ color: "#FFFDF0" }}>Новости центра</h2>
            </div>
          </div>
            ...
            {news.map((n, i) => (
              ...
              >
                <div className="w-full sm:w-40 h-40 shrink-0 overflow-hidden rounded relative">
                  {n.category && (
                    <div className="absolute top-2 left-2 z-10 bg-brand-blue-dark text-white text-[10px] font-bold uppercase px-2 py-1 rounded shadow-sm">
                      {n.category}
                    </div>
                  )}
                  <ImageWithFallback
                ...
                <div className="flex flex-col justify-center">
                  ...
                  <h3 className="text-lg font-bold mb-2 group-hover:text-brand-blue-dark transition-colors line-clamp-2 leading-snug">
                    {n.title}
                  </h3>
                  ...
                  <button 
                    ...
                    className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-muted-foreground group-hover:text-brand-blue-dark transition-colors"
                  >
        ...
        <div className="max-w-5xl mx-auto">
          <div className="flex items-end justify-between mb-10">
            <div>
              ...
              <h2 className="text-3xl lg:text-4xl font-bold">Школьные движения</h2>
            </div>
            <Link to="/students" className="hidden md:flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground transition-colors">
              Школьникам <ChevronRight className="w-4 h-4" />
            </Link>
          </div>
            ...
            {movements.map((m, i) => (
              ...
              >
                ...
                <div className="flex items-center justify-between mb-5 relative z-10">
                  ...
                  <span className={`text-[10px] font-mono uppercase tracking-widest border px-2 py-0.5 rounded ${m.tagColor}`}>
                    {m.tag}
                  </span>
                </div>
                <h3 className="text-xl font-bold mb-3 relative z-10">{m.title}</h3>
                <p className="text-sm text-muted-foreground mb-6 leading-relaxed relative z-10">{m.desc}</p>
                <div className="flex items-center gap-1 text-xs font-bold text-muted-foreground group-hover:text-foreground transition-colors relative z-10">
              ...
              <div className="overflow-y-auto">
                <div className="relative w-full bg-secondary/30">
                  ...
                  {selectedNews.category && (
                    <div className="absolute bottom-6 left-6 bg-brand-blue-dark text-white text-xs font-black uppercase px-4 py-2 rounded-xl shadow-xl">
                      {selectedNews.category}
                    </div>
                  )}
                </div>
                ...
                <div className="p-8 sm:p-10">
                  <div className="flex items-center gap-3 text-xs font-mono text-muted-foreground mb-6 bg-secondary/50 w-fit px-3 py-1.5 rounded-full">
                    <Calendar className="w-4 h-4 text-brand-blue-dark" />
                    <span className="font-bold uppercase tracking-widest">{selectedNews.date}</span>
                  </div>
                  
                  <h2 className="text-2xl sm:text-4xl font-black text-brand-blue-dark leading-tight mb-8">
                    {selectedNews.title}
                  </h2>
                  ...
                  <div className="mt-12 pt-8 border-t border-border flex items-center justify-between">
                    ...
                    <button
                      onClick={() => setSelectedNews(null)}
                      className="text-xs font-black uppercase tracking-widest text-brand-blue-dark hover:underline"
                    >
                      Закрыть новость
                    </button>
                  </div>
